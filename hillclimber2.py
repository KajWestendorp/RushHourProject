...
for trial in range(num_trials):
    ...
    if solver.is_solution():
        ...
        hillclimber2 = HillClimber2(solved_grid)
        best_moves = hillclimber2.run(iterations=iterations, verbose=False)

        # Store results
        hillclimber2_results.append((len(random_moves), len(best_moves) if best_moves else "Not solved"))
    else:
        print(f"Trial {trial + 1}: Algorithm failed to find a solution, skipping trial.")
        hillclimber2_results.append(("Not solved", "Not solved"))

# Save results
df_hillclimber = pd.DataFrame(hillclimber2_results, columns=['random_moves', 'hillclimber2_moves'])
df_hillclimber.to_csv(f"{num_trials}trials_{iterations}iterations_hillclimber2_{algorithm}_boardposition{boardposition}.csv", index=False)
print("\nHill Climber2 trials completed")
